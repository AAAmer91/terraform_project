...
# Data block to find any existing EC2 instance
...
# Use an existing EC2 instance or create a new one
...
output "ec2_public_ip" {
  value = length(data.aws_instance.existing_instance.ids) > 0 ? data.aws_instance.existing_instance.public_ip : aws_instance.my_ec2_instance.public_ip
}
